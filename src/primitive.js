...
const primitive = (function() {
    ...
    return {
        ...
        Cube: function(p,L,m,o) {
            const geometry = new THREE.BoxGeometry( L,m,o );
        ...
        Cylinder: function(p0,p1,r,l) {
            const u = p1.clone().sub(p0); // axe du cylindre
            const L = u.length(); // longueur du cylindre
            const geometry = new THREE.CylinderGeometry(r,l,L,20);
        ...
        specialGeometry: function(l,L){
            var geometry = new THREE.Geometry();
            var vectors=[];
            vectors.push(Vector3(-0.5*L,0.5*l,0));
            vectors.push(Vector3(0.5*L,0.5*l,0));
            for(var i=1;i<100;i++){
                vectors.push(Vector3(0.5*L+0.5*l*Math.cos(Math.PI/2-i*Math.PI/100),Math.sin(Math.PI/2-i*Math.PI/100)*l/2,0));
            }
            vectors.push(Vector3(0.5*L,-0.5*l,0));
            vectors.push(Vector3(-0.5*L,-0.5*l,0));
            for(var i=1;i<100;i++){
            ...
            geometry.faces.push( new THREE.Face3(0 , 1 , len/2+1 ));
            geometry.faces.push( new THREE.Face3(1 , len/2 , len/2+1 ) );
            geometry.faces.push( new THREE.Face3(len/2+1 , 1 , 0 ));
            geometry.faces.push( new THREE.Face3(len/2+1 , len/2 , 1 ) );

            for(let i=3; i<=len/2 ; i++){
                var f1 = new THREE.Face3(1 , i-1 ,i );
                var f2 = new THREE.Face3( 0 , i+len/2-2 , i+len/2-1  );
                var f3 = new THREE.Face3( i, i-1 , 1 );
                var f4 = new THREE.Face3( i+len/2-1 , i+len/2-2 , 0  );
